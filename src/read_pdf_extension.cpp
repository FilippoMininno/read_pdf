...
static void CollectFields(QPDFObjectHandle field,
                          const std::string &prefix,
                          std::vector<std::pair<std::string, std::string>> &out,
                          QPDFObjGen::set &visited,
                          int depth) {
    if (depth > kMaxFieldDepth) return;
    if (!visited.add(field.getObjGen())) return;  // cycle detected
    ...
    if (!field.hasKey("/FT") || name.empty()) return;

    std::string value;
    if (field.hasKey("/V")) {
        auto v = field.getKey("/V");
        if (v.isString())    value = v.getUTF8Value();
        else if (v.isName()) value = v.getName();
    }
...
ReadFormFieldsFromMemory(const std::vector<char> &buf,
    ...
    if (!root.hasKey("/AcroForm")) return out;
    auto acroform = root.getKey("/AcroForm");
    if (!acroform.hasKey("/Fields")) return out;
...
static unique_ptr<FunctionData> PdfFormBind(ClientContext &context,
    ...
    if (!FileSystem::HasGlob(input_path) && fs.DirectoryExists(input_path)) {
        ...
        for (auto &f : matches) {
            if (f.path.size() >= 4 &&
                f.path.compare(f.path.size() - 4, 4, ".pdf") == 0) {
                bind_data->paths.push_back(f.path);
            }
        }
        std::sort(bind_data->paths.begin(), bind_data->paths.end());
    ...
    if (first_fields.empty()) {
        throw BinderException(
            "read_pdf: no readable PDF with AcroForm fields found at '%s'", input_path);
    }
    ...
    if (schema_idx > 0) {
        bind_data->paths.erase(bind_data->paths.begin(),
                               bind_data->paths.begin() + static_cast<ptrdiff_t>(schema_idx));
    }
...
PdfFormInitGlobal(ClientContext &context, TableFunctionInitInput &input) {
...
static void PdfFormScan(ClientContext &context, TableFunctionInput &data, DataChunk &output) {
    ...
    while (row < STANDARD_VECTOR_SIZE && gstate.file_idx < bind_data.paths.size()) {
        ...
        for (idx_t col = 0; col < bind_data.column_names.size(); col++) {
            ...
            output.SetValue(col + 1, row,
                (it == values.end() || it->second.empty()) ? Value() : Value(it->second));
        }
        row++;
    }
...
static void LoadInternal(ExtensionLoader &loader) {
    ...
    // Table function
    TableFunction read_pdf("read_pdf", {LogicalType::VARCHAR},
                           PdfFormScan, PdfFormBind, PdfFormInitGlobal);
